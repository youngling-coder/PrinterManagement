...
def get_printers_by_location_id(location_id: int):
    with get_db_cursor() as cursor:
        stmt = cursor.execute(f"SELECT * FROM {TABLE_NAME} WHERE location_id = (?)", (location_id,))
...
def create_printer(data: dict):
    ...
    try:
        with get_db_cursor() as cursor:
            cursor.execute(f"INSERT INTO {TABLE_NAME} ({", ".join(data.keys())}) VALUES (?,?,?,?,?,?)", tuple(data.values()))
...
def update_printer(id: int, data: dict):
    try:
        with get_db_cursor() as cursor:
            ...
            cursor.execute(f"UPDATE {TABLE_NAME} SET {set_clause} WHERE id = ?", (*data.values(), id))
