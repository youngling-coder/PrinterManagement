...
class CliApplication:
    ...
    def remove_printer(self):
        printer, location = cli_input.select_printer_data(self.service.get_all_locations())
        if cli_input.get_yn_confirmation(f"Drucker '{printer.name}' wirklich entfernen?"):
    ...
    def install_printer(self):
        ...
        if cli_input.get_yn_confirmation(f"Installation für '{printer.name}' fortsetzen?"):
            ...
            cli_output.print_success(f"Drucker '{printer.name}' wurde erfolgreich installiert!")


    def add_location(self):
        name = cli_input.get_generic_input("Name des neuen Standorts eingeben: ", empty=False)
    ...
    def remove_location(self):
        ...
        if cli_input.get_yn_confirmation(f"Standort '{location_name}' wirklich entfernen?"):
            self.service.remove_location(location_name)
            cli_output.print_success("Standort wurde erfolgreich entfernt.")

    def restore_backup(self):
        path = cli_input.get_generic_input("[Optional] Pfad zur Backup-Datei eingeben: ")
        if cli_input.get_yn_confirmation("Aktuelle Daten werden überschrieben. Fortfahren?"):
