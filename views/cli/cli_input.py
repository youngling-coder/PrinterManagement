...
def get_integer_input(prompt: str, min_val: int, max_val: int, optional: bool = False) -> Optional[int]:
    ...
    while True:
        ...
        try:
            ...
            if min_val <= value_int <= max_val:
                ...
                cli_output.print_error(f"Bitte geben Sie eine Zahl zwischen {min_val} und {max_val} ein.")
        except ValueError:
            cli_output.print_error("Ungültige Eingabe. Bitte geben Sie eine ganze Zahl ein.")
...
def get_location_index_input(locations: List[Location], optional: bool = False) -> Optional[int]:
...
def input_printer_data(locations: List[Location]) -> Tuple[Dict[str, Any], str]:
    ...
    printer_data = {
        "dns": get_generic_input("DNS-Namen/IP-Adresse des Druckers eingeben: ", empty=False),
        ...
        "driver_name": get_generic_input("Name des Druckertreibers eingeben: ", empty=False),
        "driver_inf_path": get_generic_input("Pfad zur .inf-Treiberdatei eingeben: ", empty=False),
    }
