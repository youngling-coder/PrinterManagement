...
def print_menu():
    ...
    print(f'Geben Sie {Style.BRIGHT}"exit"{Style.RESET_ALL} oder drücken Sie Strg+C, um das Programm zu beenden.\n')
...
def print_location_overview(location: Location):
    ...
    for i, printer in enumerate(location.printers):
        ...
        status_icon = f"{Fore.GREEN}Ja{Style.RESET_ALL}" if status else f"{Fore.RED}Nein{Style.RESET_ALL}"
        table.add_row([
            i + 1,
            printer.name,
            printer.dns,
...
def format_locations_for_selection(locations: List[Location]) -> str:
    ...
    return "\n".join([f"{Style.BRIGHT}{i + 1}.{Style.RESET_ALL} {loc.name}" for i, loc in enumerate(locations)])