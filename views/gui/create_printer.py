...
class CreatePrinterDialog(QDialog, Ui_CreatePrinterDialog):
    def __init__(self, service: PrinterService, printer: Optional[Printer] = None, edit_mode: bool = False):
        super().__init__()
        self.setupUi(self)
    ...
    def _populate_locations(self):
        ...
        if not locations and not self.edit_mode:
            QMessageBox.warning(self, "Keine Standorte", "Bitte erstellen Sie zuerst einen Standort, bevor Sie einen Drucker hinzufügen.")
    ...
    def _load_printer_data(self):
        ...
        if location:
            ...
            self.locationComboBox.setEnabled(False) # Standort kann nicht geändert werden

    def _on_accept(self):
        # Einfache Validierung, ob alle Felder ausgefüllt sind
        fields = [self.DNSNameEdit, self.printerNameEdit, self.printerModelEdit, self.driverNameEdit, self.driverPathEdit]
        if any(not field.text().strip() for field in fields):
            QMessageBox.warning(self, "Fehlende Eingabe", "Alle Felder müssen ausgefüllt werden.")
            return
        self.accept()
    ...
    def _select_driver_file(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            path, _ = QFileDialog.getOpenFileName(self, "Treiberdatei auswählen", "", "Treiber-Informationen (*.inf)")
            if path:
